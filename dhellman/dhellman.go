...
// pair of wire messages clients use to swap public keys before deriving a
// shared symmetric key.
//
// Typical flow (both sides are symmetric):
//
//  1. Each side calls GenerateKeyPair to get an ephemeral X25519 keypair.
//  2. Each side marshals a HelloMessage{PublicKey, Nonce} and sends it
//     to the peer (e.g. as the Payload of a c2s plain handshake frame).
//  3. On receipt of the peer's HelloMessage, each side calls
//     KeyPair.ComputeShared to get the raw DH secret, then DeriveKey to
//     expand it into a symmetric session key. The two nonces are mixed
//     into the HKDF salt so the derived key binds to this exact session.
//
...
// the session and protocol context — typically salt = ourNonce||peerNonce
// and info identifies the layer (e.g. []byte("novaproto/c2s/v1")).
...
// handshake. Both peers send one. The Nonce is per-session random data
// the sender contributes to key derivation; mix both nonces into the
// HKDF salt on each side.
type HelloMessage struct {
	PublicKey [PublicKeySize]byte
	Nonce     [NonceSize]byte
}

// Wire layout: [version u16 | publicKey 32 | nonce 16] = 50 bytes.
const (
	helloVersion uint16 = 1
	helloSize           = 2 + PublicKeySize + NonceSize
)

// NewHelloMessage builds a HelloMessage from the keypair's public half
// and a freshly generated random nonce.
func NewHelloMessage(kp *KeyPair) (*HelloMessage, error) {
	if kp == nil {
		return nil, errors.New("dhellman: nil keypair")
	}
	m := &HelloMessage{PublicKey: kp.PublicKey()}
...
// Marshal serializes the HelloMessage into its fixed 50-byte wire form.
func (m *HelloMessage) Marshal() []byte {
	buf := make([]byte, helloSize)
	binary.BigEndian.PutUint16(buf[0:], helloVersion)
	copy(buf[2:], m.PublicKey[:])
	copy(buf[2+PublicKeySize:], m.Nonce[:])
	return buf
}

// UnmarshalHello parses a wire-form HelloMessage.
func UnmarshalHello(buf []byte) (*HelloMessage, error) {
	if len(buf) != helloSize {
		return nil, errors.New("dhellman: hello size mismatch")
	}
	if v := binary.BigEndian.Uint16(buf[0:]); v != helloVersion {
		return nil, errors.New("dhellman: unsupported hello version")
	}
	m := &HelloMessage{}
	copy(m.PublicKey[:], buf[2:2+PublicKeySize])
	copy(m.Nonce[:], buf[2+PublicKeySize:])
	return m, nil
}
